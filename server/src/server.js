...
app.post('/feeditem/:feeditemid/comments/', validate({ body: CommentSchema }), function(req, res) {
  ...
  if (fromUser === body.userId) {
    var newUpdate = postComment(body.feedItemId, body.userId, body.contents);
    // When POST creates a new resource, we should tell the client about it
    // in the 'Location' header and use status code 201.
    res.status(201);
    res.set('Location', '/feeditem/:feeditemid/comments/' + newUpdate.comments.length);
    for(var i=0;i<newUpdate.comments.length;i++){
    newUpdate.comments[i].author = readDocument('users', newUpdate.comments[i].author );
     } // Send the update!
    res.send(newUpdate);
  } else {
    // 401: Unauthorized.
